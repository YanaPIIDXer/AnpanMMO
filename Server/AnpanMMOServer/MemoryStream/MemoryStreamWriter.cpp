...
//コンストラクタ
MemoryStreamWriter::MemoryStreamWriter( const int _Size )
	: Size( _Size ) ,
	  CurrentPosition( 0 ) ,
	  bError( false )
{
...
bool MemoryStreamWriter::Serialize( s32 *pValue )
{
	...
	if( !Write( &NetworkValue , sizeof( s32 ) ) )
	{
...
bool MemoryStreamWriter::Serialize( u32 *pValue )
{
	...
	if( !Write( &NetworkValue , sizeof( u32 ) ) )
	{
...
bool MemoryStreamWriter::Serialize( s16 *pValue )
{
	...
	if( !Write( &NetworkValue , sizeof( s16 ) ) )
	{
...
bool MemoryStreamWriter::Serialize( u16 *pValue )
{
	...
	if( !Write( &NetworkValue , sizeof( u16 ) ) )
	{
